...
for mode in ['original','gzip-without-patch','gzip-with-patch']:
 ...
 raw=(ROOT/('boot-original.img' if mode=='original' else 'install/boot-gzip.img')).read_bytes();u.mem_write(IMG,raw)
 ...
 if mode=='gzip-with-patch':u.mem_write(B,(ROOT/'install/u-boot-gzip.bin').read_bytes()) # exact final binary, including checksum
