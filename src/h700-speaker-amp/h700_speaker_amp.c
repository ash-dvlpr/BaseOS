...
/* H700 vendor codec speaker gating and optional headphone buffer retention.
 * Built against an audited kernel profile; private ASoC layouts are not a
 * stable ABI. The generated profile and runtime checks must both match.
...
MODULE_PARM_DESC(apply, "Attach the audio fix; default is inspection only");

/* Read-only after load: config changes take effect at the next boot. */
static bool headphone_pop_fix;
module_param(headphone_pop_fix, bool, 0400);
MODULE_PARM_DESC(headphone_pop_fix, "Retain headphone output buffers at idle; default off");
...
static event_fn old_lineout_event;
static bool lineout_ready;
static struct gpio_desc *amp;
static struct gpio_chip *amp_chip;
static unsigned int amp_offset;
static int *jack_speaker_state;
static void (*vendor_gpio_set)(struct gpio_chip *, unsigned int, int);
static int (*vendor_gpio_direction_output)(struct gpio_chip *, unsigned int, int);
static bool amplifier_requested;
static DEFINE_SPINLOCK(amp_lock);
static bool notifiers_registered;

/* The vendor jack worker bypasses DAPM and even writes a transient high while
 * disabling the pin. Arbitrate both chip entry points without sleeping: these
 * callbacks can run under GPIO/pinctrl spinlocks. Every other pin passes through.
 * Keep the jack worker itself intact (some boards also swap DAC channels there).
 */
static int amp_allowed_value(unsigned int offset, int value)
{
	if (offset == amp_offset && value &&
	    (!READ_ONCE(amplifier_requested) || !READ_ONCE(lineout_ready) ||
	     READ_ONCE(*jack_speaker_state) != 1))
		return 0;
	return value;
}

static void guarded_gpio_set(struct gpio_chip *chip, unsigned int offset, int value)
{
	unsigned long flags;
	if (offset != amp_offset) {
		vendor_gpio_set(chip, offset, value);
		return;
	}
	spin_lock_irqsave(&amp_lock, flags);
	vendor_gpio_set(chip, offset, amp_allowed_value(offset, value));
	spin_unlock_irqrestore(&amp_lock, flags);
}

static int guarded_gpio_direction_output(struct gpio_chip *chip,
					 unsigned int offset, int value)
{
	unsigned long flags;
	int ret;
	if (offset != amp_offset)
		return vendor_gpio_direction_output(chip, offset, value);
	spin_lock_irqsave(&amp_lock, flags);
	ret = vendor_gpio_direction_output(chip, offset,
					  amp_allowed_value(offset, value));
	spin_unlock_irqrestore(&amp_lock, flags);
	return ret;
}
...
static int ramp_down_keep_buffers(void)
{
	...
	pr_debug("h700_retain: ramp-down rate=%lu step=%u wait=%u analog=%08x digital=%08x\n",
...
static int release_idle_buffers(void)
{
	...
	pr_debug("h700_retain: idle buffers released analog=%08x\n",
...
static int power_notify(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	pr_debug("h700_retain: PM action=%lu result=%d\n", action, ret);
...
{
	unsigned long flags;
	/* Allow no vendor high write until codec startup and settling finish. */
	if (enabled && !READ_ONCE(amplifier_requested) &&
	    READ_ONCE(*jack_speaker_state) == 1)
		msleep(settle_ms);
	spin_lock_irqsave(&amp_lock, flags);
	WRITE_ONCE(amplifier_requested, enabled);
	spin_unlock_irqrestore(&amp_lock, flags);
	enabled = enabled && READ_ONCE(lineout_ready) &&
		  READ_ONCE(*jack_speaker_state) == 1;
			 ...
			 struct snd_kcontrol *k, int event)
{
	int ret;
	if (event == SND_SOC_DAPM_PRE_PMD) {
		WRITE_ONCE(lineout_ready, false);
		ret = set_amp(false);
		if (ret)
			return ret;
		if (headphone_pop_fix && !power_preparing) {
...
static int check_identity(void)
{
	...
	/* Exported UTS data is safe to read before touching private addresses. */
	if (strcmp(init_uts_ns.name.release, H700_KERNEL_RELEASE) ||
	    strcmp(init_uts_ns.name.version, H700_KERNEL_VERSION)) {
		pr_err("h700_amp: kernel identity differs from compiled profile\n");
		return -ENODEV;
	}
	banner = (void *)h700_kernel_address(H700_LINUX_BANNER_OFFSET);
	if (!banner || strcmp(banner, H700_KERNEL_BANNER) || !stub ||
	...
	amp_chip = gpiod_to_chip(amp);
	jack_speaker_state = (void *)h700_kernel_address(H700_JACK_STATE_OFFSET);
	if (!amp_chip || !jack_speaker_state || !amp_chip->set ||
	    !amp_chip->direction_output || amp_chip->base < 0 ||
	    261 < amp_chip->base || 261 >= amp_chip->base + amp_chip->ngpio) {
		pr_err("h700_amp: unsupported amplifier GPIO chip\n");
		goto out;
	}
	if ((unsigned long)amp_chip->set !=
	    h700_kernel_address(H700_GPIO_SET_OFFSET) ||
	    (unsigned long)amp_chip->direction_output !=
	    h700_kernel_address(H700_GPIO_DIRECTION_OUTPUT_OFFSET)) {
		pr_err("h700_amp: unexpected GPIO callbacks\n");
		goto out;
	}
	amp_offset = 261 - amp_chip->base;
	vendor_gpio_set = amp_chip->set;
	vendor_gpio_direction_output = amp_chip->direction_output;
	initial_pin = gpiod_get_raw_value_cansleep(amp);
	if (initial_pin != 0 && initial_pin != 1)
		goto out;
	/* Speaker-only mode needs neither ramp metadata nor a codec clock reference. */
	if (!headphone_pop_fix) {
		ret = 0;
		goto out;
	}
...
static int __init amp_init(void)
{
	...
	/* Private layouts audited against every supported vendor kernel. */
	...
	BUILD_BUG_ON(offsetof(struct gpio_chip, direction_output) != 0x40);
	BUILD_BUG_ON(offsetof(struct gpio_chip, set) != 0x50);
	...
	if (headphone_pop_fix) {
		...
		notifiers_registered = true;
	}
	old_lineout_event = lineout->event;
	/* No codec shutdown follows attachment, so there is no ramp to delay.
	 * Playback events still apply the full vendor settling delay. */
	WRITE_ONCE(amplifier_requested, false);
	gpiod_set_raw_value_cansleep(amp, 0);
	if (gpiod_get_raw_value_cansleep(amp) != 0) {
		ret = -EIO;
		goto unlock_card;
	}
	WRITE_ONCE(lineout_ready, false);
	/* Nothing below can fail. These permanent hooks must never outlive us. */
	WRITE_ONCE(amp_chip->set, guarded_gpio_set);
	WRITE_ONCE(amp_chip->direction_output, guarded_gpio_direction_output);
	/* Close the interval in which the vendor jack worker could have written
	 * high between the initial mute and hook publication. */
	gpiod_set_raw_value_cansleep(amp, 0);
	speaker->event = speaker_event;
	lineout->event = lineout_event;
	attached = true;
	pr_info("h700_amp: callbacks attached headphone_pop_fix=%d\n", headphone_pop_fix);
unlock_card:
	mutex_unlock(&target_card->dapm_mutex);
unlock_soc:
	mutex_unlock(soc_mutex);
	if (ret && notifiers_registered) {
...
/* Callbacks reference permanent built-in devices. Deliberately non-unloadable:
 * removing this module while PCM/DAPM work is active is unsafe. Reboot to undo. */
module_init(amp_init);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("BaseOS H700 vendor speaker and headphone pop repair");
